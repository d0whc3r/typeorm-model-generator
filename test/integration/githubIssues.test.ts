    ...
    for (let folder of files) {
        ...
        describe(`#${folder}`, async function() {
            for (let dbDriver of dbDrivers) {
                ...
                switch (folder) {
                    case "39":
                        if (dbDriver == "mysql" || dbDriver == "mariadb" || dbDriver == "oracle" || dbDriver ==
                            "sqlite")
                            continue;
                        break;
                    default:
                ...
                it(dbDriver, async function() {
                    ...
                    let filesOrg = fs.readdirSync(filesOrgPathTS).filter(function(this, val) {
                        return val.toString().endsWith(".ts");
                    });
                    let filesGen = fs.readdirSync(filesGenPath).filter(function(this, val) {
                        return val.toString().endsWith(".ts");
                    });
