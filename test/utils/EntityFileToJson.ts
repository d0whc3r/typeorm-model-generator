export class EntityFileToJson {
        ...
        if (decoratorParameters.length > 0) {
            if (decoratorParameters[0] == "\"" && decoratorParameters.endsWith("\"")) {
                ...
                if (badJSON.lastIndexOf(",") == badJSON.length - 3) {
                    badJSON = badJSON.slice(0, badJSON.length - 3) + badJSON[badJSON.length - 2] +
    ...
    getColumnOptionsAndType(trimmedLine: string, col: EntityColumn) {
        ...
        if (decoratorParameters.length > 0) {
            if (decoratorParameters.search(",") > 0) {
                col.columnTypes = decoratorParameters.substring(0, decoratorParameters.indexOf(","))
                    .trim()
                    .split("|")
                    .map(function(x) {
                        return x;
    ...
    getRelationOptions(trimmedLine: string, col: EntityColumn) {
        ...
        if (decoratorParameters.length > 0) {
            ...
            if (params && params.length == 2) {
                let badJSON = decoratorParameters.substring(decoratorParameters.lastIndexOf("{"),
                    decoratorParameters.lastIndexOf("}") + 1).trim();
                if (badJSON.lastIndexOf(",") == badJSON.length - 3) {
                    badJSON = badJSON.slice(0, badJSON.length - 3) + badJSON[badJSON.length - 2] +
                        badJSON[badJSON.length - 1];
                }
                col.columnOptions = JSON.parse(
                    badJSON.replace(/(')/g, `"`).replace(/(['"])?([a-z0-9A-Z_]+)(['"])?:/g, "\"$2\": "));
            } else {
    ...
    getIndexOptions(trimmedLine: string, ind: EntityIndex) {
        ...
        if (decoratorParameters.length > 0) {
            ...
            if (containsName) {
                ind.indexName = decoratorParameters.slice(decoratorParameters.indexOf("\"") + 1,
                    decoratorParameters.substr(decoratorParameters.indexOf("\"") + 1).indexOf("\""));
            }
            if (containsTables) {
                let columnsStr = decoratorParameters.slice(decoratorParameters.indexOf("[") + 1,
            ...
            if (containsOptions) {
                let optionsStr = decoratorParameters.slice(decoratorParameters.indexOf("{") + 1,
                    decoratorParameters.indexOf("}"));
        ...
        for (let line of lines) {
            ...
            if (trimmedLine.length == 0)
                ...
                if (trimmedLine.startsWith("import")) {
                    ...
                    retVal.entityName = trimmedLine.substring(trimmedLine.indexOf("class") + 5,
                        trimmedLine.lastIndexOf("{")).trim().toLowerCase();
                    isInClassBody = true;
                    continue;
                ...
                if (trimmedLine.startsWith("@Column")) {
                    ...
                    if (retVal.indicies.length > 0 && retVal.indicies[retVal.indicies.length - 1].columnNames.length ==
                        0) {
                        retVal.indicies[retVal.indicies.length - 1].columnNames.push(
